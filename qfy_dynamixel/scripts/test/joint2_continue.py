...
def pub_data():
    ...
    rospy.init_node('pub_joint2',anonymous=True)
    up_slope = True
    joint2_data = Float64()
    # joint3_data = Float64()
    ...
    while not rospy.is_shutdown():
        ...
        if i < len(joint_list)-1 and up_slope:
            ...
            if i == 0 :
                up_slope = True
        joint2_data.data = joint_list[i]
        # joint3_data.data = joint_list[i]
        pub_joint2.publish(joint2_data)
        # pub_joint3.publish(joint3_data)
        rospy.loginfo_throttle(0.1, "joint2 : %f"%(joint2_data.data))
        rate.sleep()
